...
def generate_sine_wave(frequency, duration, sample_rate=48000, amplitude=0.5):
...
def generate_white_noise(duration, sample_rate=48000, amplitude=0.1):
    noise = np.random.normal(0, amplitude, int(sample_rate*duration))
    
    # Convert mono to stereo by duplicating channels
    stereo_noise = np.column_stack((noise, noise)).flatten()
    return stereo_noise
...
def play_sound_blocking(sound, sample_rate=48000):
    p = pyaudio.PyAudio()
